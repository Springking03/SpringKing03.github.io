//Xem chi tiết
 document.querySelectorAll('.quick-view').forEach(btn => {
            ...
            // Lấy dữ liệu từ các thuộc tính và thẻ con của sản phẩm
            const name = product.getAttribute('data-name');
            const price = parseFloat(product.getAttribute('data-price'));
            const img = product.getAttribute('data-img');
            const category = product.querySelector('.product-category')?.innerText || '';
            const oldPrice = product.querySelector('.product-old-price')?.innerText || '';
            const discount = product.querySelector('.product-label .sale')?.innerText || '';

            const productDetail = { name, price, img, category, oldPrice, discount };
